...
export function LevelSingle() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const res = await LevelService.getById(id);
        const data = unwrap(res, 'level');
  ...
  const handleDelete = async () => {
    ...
    if (!ok) return;
    try {
      await LevelService.remove(item.id);
  ...
  if (!item) return <div className="text-gray-500">Level not found.</div>;

  return (
    <div>
      <div className="mb-6 flex items-center gap-4">
        <Link to="/levels" className="text-[var(--color-accent)] hover:underline">
          ← Back to Levels
        </Link>
      </div>
      ...
      <div className="mt-4 flex gap-2">
        <Link to={`/levels?edit=${item.id}`}>
          <Button variant="secondary" className="!p-2" title="Edit level" aria-label="Edit level">
            <IconEdit />
          </Button>
        </Link>
        <Button variant="danger" className="!p-2" title="Delete level" aria-label="Delete level" onClick={handleDelete}>
          <IconTrash />
        </Button>
      </div>
    </div>
